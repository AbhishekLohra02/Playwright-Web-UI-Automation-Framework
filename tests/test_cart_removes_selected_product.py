...
def test_cart_removes_selected_product(cart_with_products: CartPage) -> None:
    ...
    expect(cart_with_products.products.rows).to_have_count(REMAINING_PRODUCTS)
    expect(cart_with_products.products.row(BACKPACK)).to_have_count(0)
    expect(cart_with_products.products.row(BIKE_LIGHT)).to_be_visible()
    expect(cart_with_products.header.cart_badge).to_have_text(str(REMAINING_PRODUCTS))
